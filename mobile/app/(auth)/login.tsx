...
export default function LoginScreen() {
  ...
  const handleWebCode = async () => {
    ...
    try {
      ...
      if (!res.ok) { Alert.alert("Error", data.error ?? "Código inválido o expirado"); return; }
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error?.includes("Google")) {
          Alert.alert(
            "Cuenta de Google",
            "Esta cuenta fue creada con Google y no tiene contraseña.\n\nUsá el código de la web: andá a Perfil → Código para mobile.",
            [
              { text: "Cancelar", style: "cancel" },
              { text: "Ingresar código", onPress: () => setShowCodeInput(true) },
            ],
    ...
    >
      ...
      >
        ...
        <View style={s.card}>
          ...
          {/* Google button (info only in Expo Go) */}
          <TouchableOpacity style={s.googleBtn} onPress={handleGoogleInfo}>
          ...
          {showCodeInput && (
            <View style={[s.codeBox, { borderColor: t.border, backgroundColor: t.input }]}>
              ...
              <TouchableOpacity
                style={[s.btn, { backgroundColor: t.text, marginBottom: 0 }]}
                onPress={handleWebCode}
              ...
              <Text style={[s.codeHint, { color: t.muted }]}>
                Web → Perfil → "Generar código para mobile"
              </Text>
            </View>
          )}
          ...
          <View style={s.divider}>
            ...
            <Text style={[s.dividerText, { color: t.muted }]}>o ingresá con email</Text>
            <View style={[s.dividerLine, { backgroundColor: t.border }]} />
          </View>
          ...
          <TouchableOpacity onPress={() => router.push("/(auth)/register")}>
            <Text style={[s.link, { color: t.muted }]}>
              ¿No tenés cuenta? <Text style={[s.linkBold, { color: t.salmon }]}>Crear cuenta</Text>
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
...
const styles = (t: ReturnType<typeof useTheme>) => StyleSheet.create({
  ...
  googleBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, borderWidth: 1.5, borderColor: t.border, borderRadius: 14, padding: 13, backgroundColor: t.input, marginBottom: 12 },
  ...
  dividerText: { fontSize: 11, color: t.muted },
